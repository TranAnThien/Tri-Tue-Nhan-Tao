...
def run_algorithm(algo):
    ...
    if algo == "BFS":
        ...
        solution = solver.backtracking_search(start_state, goal_state)
    elif algo == "AC-3":
        final_domains, execution_log = solver.ac3()
    ...
    if algo != "No Observation Search" and algo != "Partial Observation Search" and algo != "Generate and Test" and algo != "AC-3":
        ...
        display_ac3_visualization(final_domains, execution_log, domains)
    else:
...
def display_ac3_visualization(final_domains, execution_log, initial_domains_for_ac3):
    ...
    tk.Label(constraints_info_frame, 
             text="Ràng buộc 2: Giá trị Ô_i < Giá_trị Ô_j nếu chỉ số i < j.",
             font=("Arial", 12, "italic")).pack(anchor="n", padx=10)
...
def run_all_performance_tests(current_start_state, current_goal_state):
    global performance_tree, performance_test_button
    if not performance_tree: return
    ...
    for group_name, algo_list in PERFORMANCE_ALGO_GROUPS.items():
        group_id = performance_tree.insert("", tk.END, text=group_name, open=True, tags=('group_header',))
    ...
    performance_test_button.config(state=tk.NORMAL, text="Chạy Tất cả Kiểm tra Hiệu suất")
    messagebox.showinfo("Hoàn tất", "Đã chạy xong tất cả các kiểm tra hiệu suất!")
...
def show_performance_window():
    ...
    try:
        ...
        if not is_valid_state(current_start_for_test) or not is_valid_state(current_goal_for_test):
            ...
            perf_window.destroy(); return
    except Exception as e:
        messagebox.showerror("Lỗi Đọc Trạng Thái", f"Không thể đọc Start/Goal State: {e}", parent=perf_window)
        perf_window.destroy(); return

    top_controls_frame = tk.Frame(perf_window, pady=10); top_controls_frame.pack(fill=tk.X)
    performance_test_button = tk.Button(top_controls_frame, text="Chạy Tất cả Kiểm tra Hiệu suất", 
        command=lambda: run_all_performance_tests(current_start_for_test, current_goal_for_test),
        font=("Arial", 12, "bold"), bg="#4CAF50", fg="white", width=30)
    performance_test_button.pack(pady=5)

    tree_frame = tk.Frame(perf_window); tree_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=10)
    columns = ("algo", "time", "steps", "nodes", "found")
    performance_tree = ttk.Treeview(tree_frame, columns=columns, show="headings")
    performance_tree.heading("algo", text="Thuật toán"); performance_tree.heading("time", text="Thời gian (s)")
    performance_tree.heading("steps", text="Số bước"); performance_tree.heading("nodes", text="Số nút duyệt")
    performance_tree.heading("found", text="Tìm thấy?")
    performance_tree.column("algo", width=250, anchor="w"); performance_tree.column("time", width=100, anchor="center")
    performance_tree.column("steps", width=100, anchor="center"); performance_tree.column("nodes", width=120, anchor="center")
    ...
    performance_tree.configure(yscrollcommand=vsb.set); vsb.pack(side="right", fill="y"); performance_tree.pack(side="left", fill="both", expand=True)
    ...
    perf_window.transient(root); perf_window.grab_set()
...
backtracking_search_btn = tk.Button(button_frame, text="Backtracking search", width=btn_width, command=lambda: run_algorithm("Backtracking search"), bg="#ff9999", fg="white", font=("Arial", 12, "bold"))
...
ac3_btn = tk.Button(button_frame, text="AC-3", width=btn_width, command=lambda: run_algorithm("AC-3"), bg="#7ED957", fg="white", font=("Arial", 12, "bold"))
